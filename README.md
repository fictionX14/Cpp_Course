...
- `Cpp_Systems_Review.md` — accumulated C++ systems knowledge (migrated from prior prep; seeds `concepts/`).
- `Problem_Solving_Framework.md` — the coding-portion checklist.
- `concepts/` — durable per-concept reference notes (grow over time).
- `code/` — all code, by phase; `code/prior_work/` holds earlier C++ solutions.
- `quizzes/` — quiz questions + answers + scores.
- `resources/` — curated videos/courses mapped to phases.

## Mirrors
- **Notion** "C++ Competency Roadmap" — visual dashboard (phases checklist + daily log).
